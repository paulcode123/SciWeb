import { QuestionSystem } from '/static/QuestionSystem.js';

// export Intro and MineSweeper
export { Intro, MineSweeper };
...
class Cell
{
    constructor (grid, index, x, y)
    {
        ...
        this.tile.on('pointerdown', (pointer) => {
            if (this.grid.scene.canClickCell) {
                this.onPointerDown(pointer);
            } else {
                this.grid.scene.questionSystem.displayQuestion();
            }
        });
    ...
    {
        this.grid.scene.canClickCell = false;
...
class Intro extends Phaser.Scene
{
    ...
    preload ()
    {
        this.load.setBaseURL('/static/games/minesweeper');
        ...
        this.load.image('win95', 'logoPinkWhite.png');
    }
...
class MineSweeper extends Phaser.Scene
{
    ...
    create ()
    {
        this.questionSystem = new QuestionSystem(this, this.questions);
        
        this.grid = new Grid(this, this.width, this.height, this.bombs);
        this.grid.board.setScale(3);
        this.grid.board.setPosition(
            200, 100
        );

        this.canClickCell = true;
    }

    handleAnswer (isCorrect)
    {
        this.canClickCell = isCorrect;
        if (!isCorrect) {
